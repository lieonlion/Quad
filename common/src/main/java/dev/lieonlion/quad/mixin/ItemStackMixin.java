...
public abstract class ItemStackMixin {
    ...
    private boolean applyTagsImmuneDamages(boolean original, DamageSource damage) {
        if (is(QuadItemTags.IMMUNE_FIRE) && damage.is(DamageTypeTags.IS_FIRE)) {
            return false;
        } if (is(QuadItemTags.IMMUNE_LIGHTNING) && damage.is(DamageTypeTags.IS_LIGHTNING)) {
            return false;
        } if (is(QuadItemTags.IMMUNE_EXPLOSION) && damage.is(DamageTypeTags.IS_EXPLOSION)) {
            return false;
        } if (is(QuadItemTags.IMMUNE_CACTUS) && damage.is(QuadDamageTypeTags.IS_CACTUS)) {
            return false;
        } return original;
    }
}